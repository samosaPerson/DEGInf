...
class MySampler():
    ...
    def sample_blocks(self, g, seed_nodes, exclude_eids=None):
        ...
        #(u,v) = frontier.edges()
        num_chunks = max(min(-(-u.shape[0] // self.chunk_size), self.max_num_chunks), 1)
...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunking, cache_size, GNN, verbose, nsz, lat):
    ...
    if GNN == 'SAGE':
        ...
        sampler = MySampler(num_layers, graph, chunk_size = chunk_size, compact_threshold = 1000000, max_num_chunks = 10)
    elif GNN == 'GAT':
        model = GAT2(feats.shape[1],256, 47, num_layers, 1).to(device = 'cuda:0')
        sampler = MySampler(num_layers, graph, chunk_size = chunk_size, compact_threshold = 1000000, max_num_chunks = 1)
    model.eval()
    if not(feats.is_pinned()):
        cudart = torch.cuda.cudart()
        rt = cudart.cudaHostRegister(feats.data_ptr(), feats.numel() * feats.element_size(), 0)
    ...
    c_nids = torch.sort(in_degs, descending = True)[1]
    sampler0 = dgl.dataloading.MultiLayerFullNeighborSampler(1)
    cached = torch.zeros(in_degs.shape[0], dtype=torch.bool, device=device)
    for k in range(20000):
        seeds = c_nids[k*100000:(k+1)*100000].to(device=device)
        blocks = sampler0.sample_blocks(graph, seeds)
        cached[blocks[0]] = True
        s = torch.sum(cached)
        if s>cache_size:
            break
    print(k, s.cpu().item())
    cached_feats = torch.squeeze(torch.nonzero(cached)).cpu()
    cache_size = cached_feats.shape[0]
    del sampler0
    del c_nids
    del cached
    gc.collect()
    ...
    if chunk_size < 100000000:
        if GNN == 'GAT':
            ...
            input_features = torch.empty((max(int(chunk_size/2.5) + 2000000, 6000000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
    ...
    if not(failure):
        ...
        if (e < l/500):
            ...
            lat[7] = Ti
            lat[8] = l/n2
            print('Server: average latency = ', lat[0].item(), '(ms) failures =', e, [Ts, Tt, Ti], n1, n2, sampler.t_c, mem_m/1024/1024)
        else:
...
if __name__ == '__main__':
    ...
    for i in range(len(arrival_rates)):
        ...
        for j in range(cnds.shape[1]):
            ...
            chunk_size = int(cnd[1].item() * 1.05)
            opt = cnd[0].item()
            cache_size = (torch.cuda.get_device_properties(0).total_memory) - cnd[2].item()
            cache_size = int(0.90 * cache_size/(feats.shape[1] * feats.element_size()))
    ...
    for i in range(len(arrival_rates)):
        ...
        for j in range(cnds.shape[1]):
            ...
            opt = cnd[0].item()
            cache_size = (torch.cuda.get_device_properties(0).total_memory) - cnd[2].item()
            cache_size = int(0.80 * cache_size/(feats.shape[1] * feats.element_size()))
            number_of_requests = 100 * arrival_rate
