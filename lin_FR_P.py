...
class MySampler():
    ...
    def sample_blocks(self, g, seed_nodes, exclude_eids=None):
        ...
        #(u,v) = frontier.edges()
        num_chunks = max(min(-(-u.shape[0] // self.chunk_size), 10), 1)
        num_chunks = 1
...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunking, cache_size, GNN, verbose, nsz, lat):
    ...
    if chunk_size < 100000000:
        if GNN == 'GAT':
            input_features = torch.empty((max(int(chunk_size/4.5) + 2500000, 7000000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
        else:
            input_features = torch.empty((max(int(chunk_size/3) + 2500000, 6000000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
    ...
    if not(failure):
        ...
        #    pickle.dump(dist, f)
        #mem_usage.sort()
        m_max = torch.cuda.max_memory_allocated()/1024/1024
        ...
        if (e < l/500):
            ...
            lat[7] = Ti
            lat[8] = l/n2
            print('Server: average latency = ', lat[0].item(), '(ms) failures =', e, [Ts, Tt, Ti], n1, n2, sampler.t_c, m_max)
        else:
...
if __name__ == '__main__':
    ...
    arrival_rates = [50, 100, 150, 200, 250, 300, 350, 400, 450]
    ...
    for i in range(len(arrival_rates)):
        ...
        for j in range(cnds.shape[1]):
            ...
            cache_size = int(0.85 * cache_size/(feats.shape[1] * feats.element_size()))
    ...
    print('\n\n\n')
    GNN = 'GAT'
    arrival_rates = [15, 30, 45, 60, 75, 90]
    res = torch.zeros((len(arrival_rates), 10), dtype = torch.float)
    temp = torch.zeros(cnds.shape[1], 9)
    for i in range(len(arrival_rates)):
        print('\n')
